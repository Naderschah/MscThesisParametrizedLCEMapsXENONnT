...
There is a numpy version and a jax version available that mirror one another, for numpy find this file with the _np.py file ending
...
def get_input_functions(
    ...
    pmt_pos_reshape = jnp.reshape(pmt_pos_top, (1, n_pmts, 2))
...
def direct_detection_input(pmt_pos_top):
    ...
    def compute_radial_distance(X):
        ...
        """
        X_reshaped = jnp.reshape(X, (X.shape[0], 1, 2))
        distances = jnp.linalg.norm(pmt_pos_reshape - X_reshaped, axis=2, keepdims=True)
        return distances
...
def wall_reflection_input(pmt_pos_top, 
    ...
    def compute_event_wall_dist(X):
        ...
        event_exp = jnp.repeat(X, repeats=n_pmts, axis=0)               # [N * n_pmts, 2]
        pmts_exp = jnp.tile(pmt_pos_top, (batch_size, 1))               # [N * n_pmts, 2]

        theta_event = jnp.arctan2(event_exp[:, 1], event_exp[:, 0])     # [N * n_pmts]
        theta_pmt = jnp.arctan2(pmts_exp[:, 1], pmts_exp[:, 0])         # [N * n_pmts]
        theta = jnp.arctan2(jnp.sin(theta_event + theta_pmt),
                            jnp.cos(theta_event + theta_pmt)) / 2.0     # [N * n_pmts]

        def compute_loss(th):
            W = tpc_r * jnp.stack([jnp.cos(th), jnp.sin(th)], axis=1)   # [N * n_pmts, 2]
            d_event = jnp.linalg.norm(W - event_exp, axis=1)            # [N * n_pmts]
            d_pmt = jnp.linalg.norm(W - pmts_exp, axis=1)               # [N * n_pmts]
        ...
        def body_fn(carry):
            ...
            move = jnp.where(better_plus, step,
                             jnp.where(better_minus, -step, 0.0))       # [N * n_pmts]
            theta_new = theta + move
            step_new = jnp.where(jnp.any(move != 0.0), step, step * 0.5)
        ...
        W_final = tpc_r * jnp.stack([jnp.cos(theta_final), jnp.sin(theta_final)], axis=1)  # [N * n_pmts, 2]
        d_event = jnp.linalg.norm(W_final - event_exp, axis=1)  # [N * n_pmts]
        d_pmt = jnp.linalg.norm(W_final - pmts_exp, axis=1)     # [N * n_pmts]

        result = jnp.stack([d_event, d_pmt], axis=1)            # [N * n_pmts, 2]
        return result.reshape((batch_size, n_pmts, 2))          # [N, n_pmts, 2]

    return compute_event_wall_dist

def make_perpendicular_wire_input(pmt_pos, pmt_r) -> Callable[[jnp.ndarray], jnp.ndarray]:
    """
    Returns a function f(events: [N, 2]) -> [N, n_pmts, 1]
    representing the total fraction of wire obstructing each event→PMT path.
    """
    angle = jnp.float32( -(jnp.pi / 3) + (jnp.pi / 2) ) # scalar
    rot_matrix = jnp.array([[jnp.cos(angle), -jnp.sin(angle)],
                            [jnp.sin(angle),  jnp.cos(angle)]], dtype=jnp.float32)  # [2, 2]
    pmt_rot = jnp.asarray(pmt_pos, dtype=jnp.float32) @ rot_matrix                              # [n_pmts, 2]

    # Constants
    h = jnp.float32(0.027)
    H = jnp.float32(68.58 / 10)
    anode_r = jnp.float32(0.0304 / 2)
    dz = h + H
    _pmt_r = jnp.float32(pmt_r)

    wire_start = jnp.reshape(jnp.array([31.8, -31.8, 28.3, -28.3]) - anode_r, (1, 4), dtype=jnp.float32)  # [1, 4]
    wire_end   = jnp.reshape(jnp.array([31.8, -31.8, 28.3, -28.3]) + anode_r, (1, 4), dtype=jnp.float32)  # [1, 4]

    def compute_n_perp_wires_in_way(events):
        # events: [N, 2]
        events_rot = events @ rot_matrix                     # [N, 2]

        grad_minus = jnp.abs((pmt_rot[:, 0] - _pmt_r) - events_rot[:, 0, None]) / dz  # [N, n_pmts]
        grad_plus  = jnp.abs((pmt_rot[:, 0] + _pmt_r) - events_rot[:, 0, None]) / dz  # [N, n_pmts]

        bounds_minus = events_rot[:, 0, None] - grad_minus * h  # [N, n_pmts]
        bounds_plus  = events_rot[:, 0, None] + grad_plus  * h  # [N, n_pmts]

        start_overlap = jnp.maximum(bounds_minus[..., None], wire_start)  # [N, n_pmts, 4]
        end_overlap   = jnp.minimum(bounds_plus[..., None], wire_end)     # [N, n_pmts, 4]
        overlap       = jnp.maximum(0.0, end_overlap - start_overlap)     # [N, n_pmts, 4]

        return jnp.sum(overlap, axis=-1, keepdims=True) / (2 * anode_r)   # [N, n_pmts, 1]

    return compute_n_perp_wires_in_way


def make_anode_mesh_input(pmt_pos, pmt_r) -> Callable[[jnp.ndarray], jnp.ndarray]:
    """
    Returns compute_anode_mesh_input(events: [N, 2]) -> [N, n_pmts, 2],
    with outputs = [wire obstruction fraction, normalized angle to mesh]
    """
    angle = jnp.float32(-jnp.pi / 3)  # mesh angle
    rot_matrix = jnp.array([
        [jnp.cos(angle), -jnp.sin(angle)],
        [jnp.sin(angle),  jnp.cos(angle)]
    ], dtype=jnp.float32).T  # [2, 2]

    pmt_pos = jnp.asarray(pmt_pos, dtype=jnp.float32)         # [n_pmts, 2]
    pmt_rot = pmt_pos @ rot_matrix                            # [n_pmts, 2]

    pmt_r = jnp.asarray(pmt_r, dtype=jnp.float32)             # [n_pmts]
    anode_r = jnp.float32(0.216 / 2)
    h = jnp.float32(0.027)
    H = jnp.float32(68.58 / 10)
    dz = h + H
    wire_pitch = jnp.float32(0.5)
    ...
    def compute_anode_mesh_input(events):
        ...
        diff = jnp.abs(2 * (pmt_rot[:, 0] - events_rot[:, 0, None]) * h / dz) / wire_pitch  # [N, n_pmts]
        n_wires = diff[..., None] / jnp.float32(0.025)                                    # [N, n_pmts, 1]

        # Angle component
        event_dir = jnp.arctan2(events[:, 1], events[:, 0])                                # [N]
        relative_angle = jnp.mod(event_dir - angle, jnp.pi)                                # [N]
        folded = jnp.where(relative_angle > (jnp.pi / 2),
                           jnp.pi - relative_angle,
                           relative_angle)                                                 # [N]
        norm_angle = (folded / (jnp.pi / 2)).reshape(-1, 1, 1)                             # [N, 1, 1]
        norm_angle = jnp.repeat(norm_angle, pmt_pos.shape[0], axis=1)                     # [N, n_pmts, 1]

        return jnp.concatenate([n_wires, norm_angle], axis=-1)                             # [N, n_pmts, 2]

    return compute_anode_mesh_input

def make_normalization_layer() -> Callable[[jnp.ndarray], jnp.ndarray]:
    """
    ...
    def normalize(X: jnp.ndarray) -> jnp.ndarray:
        norm = jnp.maximum(jnp.sum(X, axis=1, keepdims=True), eps)  # [batch_size, 1]
        return X / norm
...
def make_dense_layer(
    kernel: jnp.ndarray,     # [input_dim, units]
    bias: jnp.ndarray,       # [units]
    ...
    Output:
        [N, n_pmts, U]
    """
    def layer(X: jnp.ndarray) -> jnp.ndarray:
        # To ensure it operates on the last dimension only 
        h = jnp.einsum("...d,du->...u", X, kernel) + bias  # [N, n_pmts, U]
        return activation_fn(h)

    return layer

def make_mono_activations(base_activation: str) -> Callable[[jnp.ndarray], jnp.ndarray]:
    """
    ...
    where:
        ...
        h(x) = saturated
    """
    w_convex = jnp.float32(7.0)
    w_concave = jnp.float32(7.0)
    w_saturated = jnp.float32(2.0)
    total = jnp.float32(16.0)
    ...
    if base_activation == 'tanh':
        ...
        f = jnp.exp
        g = lambda x: -jnp.exp(-x)
        h = jax.nn.sigmoid

    elif base_activation == 'relu':
        f = jax.nn.relu
        g = lambda x: -jax.nn.relu(-x)
        h = jax.nn.sigmoid

    elif base_activation == 'sigmoid':
        f = jax.nn.sigmoid
        g = lambda x: -jax.nn.sigmoid(-x)
        h = jax.nn.sigmoid
...
# ------------------------------ Special Layers  ---------------------------------
# --------------------------------------------------------------------------------
def make_I0_layer(i0: jnp.ndarray) -> Callable[[jnp.ndarray], jnp.ndarray]:
    """
    ...
    """
    i0 = jnp.asarray(i0, dtype=jnp.float32)  # [n_pmts]

    def apply_i0(X: jnp.ndarray) -> jnp.ndarray:
        return X * i0  # broadcasting over batch dim

    return apply_i0

def make_radial_lce_layer(params: Sequence[float]) -> Callable[[jnp.ndarray], jnp.ndarray]:
    """
    ...
    """
    p, d, a, b = map(jnp.float32, params)  # ensure constants

    def radial_lce(X: jnp.ndarray) -> jnp.ndarray:
        # X: [batch_size, n_pmts], interpreted as rho
        return (
            (1.0 - b) / jnp.power(1.0 + jnp.square(X / d), p)
            + a * X
            + b
...
def make_lut_table_with_std(
        n_pe_domain: jnp.ndarray,
        n_ph_domain: jnp.ndarray,
        x_domain: jnp.ndarray,
        sigma_domain: jnp.ndarray,
    ...
    def compute_binomial_factors():
        ...
        k_safe = jnp.clip(k, 0, n_ph_domain[:, None])
        log_binom = jax.scipy.special.gammaln(n_ph_domain[:, None] + 1) \
                  - jax.scipy.special.gammaln(k_safe + 1) \
                  - jax.scipy.special.gammaln(n_ph_domain[:, None] - k_safe + 1)
        log_prob = log_binom + k_safe * jnp.log(p_dpe) + (n_ph_domain[:, None] - k_safe) * jnp.log(1 - p_dpe)
        return jnp.where(mask, jnp.exp(log_prob), 0.0)  # shape [n_ph, n_pe]

    def compute_poisson_factors():
        mu_vals = jnp.arange(n_mu, dtype=jnp.float32)  # shape [n_mu]
        mu = mu_vals[:, None]  # shape [n_mu, 1]
        k = n_ph_domain[None, :]  # shape [1, n_ph]
        log_p = k * jnp.log(jnp.maximum(mu, 1e-8)) - mu - jax.lax.lgamma(k + 1.0)
        return jnp.where(mu > 0, jnp.exp(log_p), jnp.where(k == 0, 1.0, 0.0))  # shape [n_mu, n_ph]
    ...
    def compute_gaussian_factors():
        ...
        mean = npe
        std = sigma * jnp.sqrt(npe + 1e-12)
        norm = jax.scipy.stats.norm.pdf(x, loc=mean, scale=std + 1e-12)
    ...
    def compute_chunked_lut(ph_slice, pe_slice):
        ...
        return jnp.sum(a * b_bcast * c_bcast, axis=(-2, -1))  # [n_mu, n_x, n_sigma]
    ...
    if reduce_ph or reduce_pe:
        result = jnp.zeros((n_mu, n_x, n_sigma), dtype=jnp.float32)
        ph_splits = jnp.array_split(jnp.arange(n_ph), 4 if reduce_ph else 1)
        pe_splits = jnp.array_split(jnp.arange(n_pe), 4 if reduce_pe else 1)
        for ph_slice in ph_splits:
            for pe_slice in pe_splits:
                result += compute_chunked_lut(ph_slice, pe_slice)
        return result
    else:
        return compute_chunked_lut(jnp.arange(n_ph), jnp.arange(n_pe))
...
def make_exact_lr(
    ...
    def exact_likelihood(x, mu, std):
        ...
        # Gaussian factor
        gaussian_std = std[:, None] * jnp.sqrt(npe_grid + 1e-12)  # [B, N_pe]
        gaussian_prob = jax.scipy.stats.norm.pdf(x[:, None], loc=npe_grid, scale=gaussian_std + 1e-12)  # [B, N_pe]

        # Binomial factor
        k = npe_grid[None, :, None] - nph_grid.T[None, None, :]  # [1, N_pe, N_ph]
        k = jnp.clip(k, 0, nph_grid.T)
        log_binom = jax.scipy.special.gammaln(nph_grid.T + 1) \
                    - jax.scipy.special.gammaln(k + 1) \
                    - jax.scipy.special.gammaln(nph_grid.T - k + 1)
        log_prob = log_binom + k * jnp.log(p_dpe) + (nph_grid.T - k) * jnp.log(1 - p_dpe)
        binom_prob = jnp.where((k >= 0) & (k <= nph_grid.T), jnp.exp(log_prob), 0.0)  # [1, N_pe, N_ph]

        # Poisson factor
        mu_exp = mu[:, None, None]
        log_p = nph_grid[None, None, :] * jnp.log(jnp.maximum(mu_exp, 1e-8)) \
                - mu_exp - jax.lax.lgamma(nph_grid[None, None, :] + 1.0)
        pois_prob = jnp.where(mu_exp > 0, jnp.exp(log_p), jnp.where(nph_grid[None, None, :] == 0, 1.0, 0.0))  # [B, 1, N_ph]

        result = gaussian_prob[:, :, None] * binom_prob[None, :, :] * pois_prob  # [B, N_pe, N_ph]
        return jnp.sum(result, axis=(1, 2))

    def gaussian_likelihood(x, mu, std):
        npe_mean = mu * (1 + p_dpe)
        npe_var = mu * (1 + p_dpe)**2 + mu * p_dpe * (1 - p_dpe) + jnp.abs(npe_mean) * std**2
        return -0.5 * ((x - npe_mean)**2 / (npe_var + 1e-6) + jnp.log(2 * jnp.pi * npe_var + 1e-6))
    ...
    def likelihood_fn(pred, observed, std):
        ...
        result = jnp.full_like(x, nan_safe_value)

        def safe(mask, values):
            return jnp.where(nan_safe, jnp.where(mask, values, nan_safe_value), values)

        result = result.at[bmap].set(safe(bmap, -2 * (gaussian_likelihood(x[bmap], mu[bmap], std[bmap])
                                                    - gaussian_likelihood(mu[bmap], mu[bmap], std[bmap]))))

        result = result.at[~bmap].set(safe(~bmap, -2 * (jnp.log(exact_likelihood(x[~bmap], mu[~bmap], std[~bmap]) + 1e-10)
                                                     - jnp.log(exact_likelihood(mu[~bmap], mu[~bmap], std[~bmap]) + 1e-10))))
...
def make_lut_trainable_std_lr(
        ...
        sigma_domain: jnp.ndarray,
        switching_signal: float,
        mle_estimator: Callable[[jnp.ndarray], jnp.ndarray],
        return_ratio: bool,
        nan_safe: bool,
        nan_safe_value: float
        ) -> Callable[[jnp.ndarray, jnp.ndarray, jnp.ndarray], jnp.ndarray]:
    def nearest_idx(grid: jnp.ndarray, values: jnp.ndarray) -> jnp.ndarray:
        diffs = jnp.abs(grid[None, :] - values[:, None])
        return jnp.argmin(diffs, axis=1)

    def lookup_likelihood(x, mu, std):
        mu_idx = jnp.clip(jnp.round(mu), 0, lut_table.shape[0] - 1).astype(jnp.int32)
    ...
    def loss_fn(pred, observed, std):
        ...
        if return_ratio:
            mle = mle_estimator(x)
            log_l1 = jnp.log(lookup_likelihood(x, mu, std) + 1e-10)
            log_l2 = jnp.log(lookup_likelihood(x, mle, std) + 1e-10)
            values = -2 * (log_l1 - log_l2)
        else:
            values = -jnp.log(lookup_likelihood(x, mu, std) + 1e-10)

        result = result.at[~bmap].set(safe(~bmap, values[~bmap]))
...
def make_lut_fixed_std_lr(
    ...
    def lookup_likelihood(x, mu, sigma_idx):
        mu_idx = jnp.clip(jnp.round(mu), 0, lut_table.shape[0] - 1).astype(jnp.int32)
        x_idx = nearest_idx(x_domain, x)
        sigma_idx = jnp.clip(sigma_idx.astype(jnp.int32), 0, lut_table.shape[2] - 1)
        return lut_table[mu_idx, x_idx, sigma_idx]
    ...
    def loss_fn(pred, observed, std_idx):
        ...
        if return_ratio:
            mle = mle_estimator(x)
            log_l1 = jnp.log(lookup_likelihood(x, mu, idx) + 1e-10)
            log_l2 = jnp.log(lookup_likelihood(x, mle, idx) + 1e-10)
            values = -2 * (log_l1 - log_l2)
        else:
            values = -jnp.log(lookup_likelihood(x, mu, idx) + 1e-10)
...
def make_likelihood_fn(
        ...
        mle_estimator: Callable[[jnp.ndarray], jnp.ndarray] = lambda x: x / 1.2,
        std: jnp.ndarray = jnp.ones((246,), dtype=jnp.float32) * 0.5,
        lut_table: Optional[jnp.ndarray] = None,
        x_domain: Optional[jnp.ndarray] = None,
        sigma_domain: Optional[jnp.ndarray] = None,
        p_dpe: Optional[float] = 0.2,
        m: Optional[int] = 5,
        z: Optional[int] = 20
        ) -> Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]:
    # Compute domains from switching signal
    max_x = int(jnp.ceil(switching_signal))
    n_pe_domain = jnp.arange(0.0, switching_signal + 5 * jnp.sqrt(switching_signal) + 2)
    n_ph_domain = jnp.arange(0.0, switching_signal / (1 + p_dpe) + 5 * jnp.sqrt(switching_signal / (1 + p_dpe)) + 2)
    ...
    def wrapped_fn(x: jnp.ndarray, mu: jnp.ndarray) -> jnp.ndarray:
        batch_size = x.shape[0]
        std_tiled = (
            jnp.tile(std[None, :], (batch_size, 1)) if mode != "LUT_fixed_std"
            else jnp.tile(jnp.arange(std.shape[0])[None, :], (batch_size, 1))
        )
...
def load_weights_dict(path: str) -> dict[str, dict[str, jnp.ndarray]]:
    """
    ...
    for full_key, val in raw.items():
        ...
        arr = jnp.array(val, dtype=jnp.float32)   # convert list → array
...
def gen_jax_model(
    ...
    def jax_model(xy: jnp.ndarray, obs: jnp.ndarray) -> jnp.ndarray:
        out = x_branch(xy)
        if include_wall:
    ...
    return jax.jit(jax_model)


# And lastly auto batching 
def make_dynamic_jax_model(
        fixed_model: Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]
    ) -> Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray]:
    # Vectorize over batch dim
    vmapped = jax.vmap(
        lambda x, y: fixed_model(x[None], y[None])[0],
